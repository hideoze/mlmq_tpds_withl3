...
unsigned CSRGraph::readFromGR(char file[], float &avg_degree, float &avg_weight) {
	std::ifstream cfile;
	cfile.open(file);
	...
	if (numEdges % 2)
		fptr32 += 1;
	index_type *edgeData = (index_type *) fptr32;
	...
	for (unsigned ii = 0; ii < nnodes; ++ii) {
		...
		for (unsigned jj = 0; jj < degree; ++jj) {
			...
			// if (edge_data[edgeindex] < 1)
			// {
			// 	printf("error! zero weight %d\n", edge_data[edgeindex]);
			// 	edge_data[edgeindex] = 1;
			// }

			total_weight += edge_data[edgeindex];
		}
	...
	avg_degree = total_degree / nnodes;
	avg_weight = total_weight / nedges;
...
unsigned CSRGraph::read(char file[], float &avg_degree, float &avg_weight) {
	return readFromGR(file, avg_degree, avg_weight);
}

unsigned writeToGR(int numNodes, int numEdges, index_type *RowPtr, index_type *ColIdx, edge_data_type *Value, char file[]) {

	std::ofstream cfile;
	cfile.open(file);

	// copied from GaloisCpp/trunk/src/FileGraph.h
	int masterFD = open(file, O_CREAT|O_RDWR, 00777);
	if (masterFD == -1) {
		printf("FileGraph::structureFromFile: unable to open %s.\n", file);
		return 1;
	}
	...
	//printf("file size %d\n",(int)(buf.st_size));
	size_t masterLength = 4 * sizeof(uint64_t) + (numNodes + 1) * sizeof(uint64_t) + (numEdges) * (sizeof(uint32_t) + sizeof(edge_data_type));

	//printf("len %d\n", masterLength);

	int _MAP_BASE = MAP_SHARED;
	//#ifdef MAP_POPULATE
	//  _MAP_BASE  |= MAP_POPULATE;
	//#endif

	ftruncate(masterFD, masterLength);

	void* m = mmap(0, masterLength, PROT_WRITE, _MAP_BASE, masterFD, 0);
	if (m == MAP_FAILED) {
		m = 0;
		printf("FileGraph::structureFromFile: mmap failed.\n");
		abort();
	...
	uint64_t version = 1;
	assert(version == 1);
	*fptr = htole64(version);
	fptr++;

	uint64_t sizeEdgeTy = sizeof(edge_data_type);
	*fptr = htole64(sizeEdgeTy);
	fptr++;
	
	*fptr = htole64(numNodes);
	fptr++;
	*fptr = htole64(numEdges);
	fptr++;
	...
	// cuda.
	unsigned nnodes = numNodes;
	unsigned nedges = numEdges;

	//printf("nnodes=%d, nedges=%d.\n", nnodes, nedges);
	// allocOnHost();

	for (unsigned ii = 0; ii < nnodes; ++ii) {
		//row_start[ii + 1] = le64toh();
		outIdx[ii] = htole64((unsigned)RowPtr[ii + 1]);
		//   //noutgoing[ii] = le64toh(outIdx[ii]) - le64toh(outIdx[ii - 1]);
		index_type degree = RowPtr[ii + 1] - RowPtr[ii];

		for (unsigned jj = 0; jj < degree; ++jj) {
			unsigned edgeindex = RowPtr[ii] + jj;

			unsigned dst = ColIdx[edgeindex];
			outs[edgeindex] = htole32(dst);
			if (dst >= nnodes)
				printf("\tinvalid edge from %d to %d at index %d(%d).\n", ii, dst, jj, edgeindex);
			...
			if (sizeEdgeTy) {
				edgeData[edgeindex] = Value[edgeindex];
			}
		}
