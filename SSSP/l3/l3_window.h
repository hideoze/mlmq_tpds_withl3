...
struct l3_window_state {
    ...
    unsigned long long budget = L3_WINDOW_MIN_CYCLES;
    bool armed = false;
    ...
    L3_WINDOW_HD action decision(unsigned long long now, bool dynamic,
        ...
        if (dynamic && elapsed >= L3_WINDOW_MIN_CYCLES &&
            (!probed || now - last_probe >= L3_WINDOW_MIN_CYCLES)) {
            last_probe = now;
            probed = true;
            return PROBE;
        }
    ...
    L3_WINDOW_HD void effective_feedback(unsigned count, unsigned effective,
        ...
        if (effective * 4u < count)
            ...
            budget = budget / 2;
        if (budget < L3_WINDOW_MIN_CYCLES) budget = L3_WINDOW_MIN_CYCLES;
    ...
    L3_WINDOW_HD bool allow(unsigned long long now, bool dynamic,
        ...
        return (dynamic && drain && now - start >= L3_WINDOW_MIN_CYCLES) || now - start >= limit;
    }
