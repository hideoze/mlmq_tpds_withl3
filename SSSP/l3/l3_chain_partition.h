...
struct l3_chain_partition_index {
    ...
    void build(int n,const int*row,const int*dst,const int*w,int begin,int end) {
        ...
        std::vector<int> incoming(end-begin),reverse(end-begin);
        ...
        for(int u=0;u<n;++u)for(int e=row[u];e<row[u+1];++e){
            ...
            if(v>=begin&&v<end&&eligible[v-begin]){
                ++incoming[v-begin];if(dst[row[v]]==u||dst[row[v]+1]==u)++reverse[v-begin];
            }
        }
        for(int u=begin;u<end;++u)eligible[u-begin]=eligible[u-begin]&&incoming[u-begin]==2&&reverse[u-begin]==2;
