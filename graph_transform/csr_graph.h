...
typedef int index_type; // should be size_t, but GPU chokes on size_t
...
struct CSRGraph {
	unsigned read(char file[], float &avg_degree, float &avg_weight);
	...
	unsigned readFromGR(char file[], float &avg_degree, float &avg_weight);
